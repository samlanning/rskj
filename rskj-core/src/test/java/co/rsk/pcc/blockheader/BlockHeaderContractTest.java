...
public class BlockHeaderContractTest {
    ...
    public void setUp() {
        ...
        world = new World();
        contract = (BlockHeaderContract) precompiledContracts.getContractForAddress(bcConfig, new DataWord(BLOCK_HEADER_CONTRACT_ADDRESS));
        ...
        // invoke transaction
        rskTx = new Transaction(config, PrecompiledContracts.BLOCK_HEADER_ADDR_STR, AMOUNT, NONCE, GAS_PRICE, GAS_LIMIT, DATA);
    ...
    private Block mineBlock(Block parent, RskAddress coinbase) {
        ...
        Block newBlock = blockGenerator.createChildBlock(parent, new ArrayList<>(), new ArrayList<>(),
                parent.getDifficulty().asBigInteger().longValue(), MIN_GAS_PRICE, parent.getGasLimit(), coinbase).cloneBlock();
